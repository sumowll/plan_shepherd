...
const schema = z.object({ connector: z.enum(['atrius','cigna']), patientId: z.string().max(250), tokenHash: z.string(), expiresAt: z.number().int(), nonce: z.string() }).strict();
...
export async function signReceipt(secret: string, connector: 'atrius' | 'cigna', patientId: string, token: string, expiresIn: number): Promise<string> {
  const value = { connector, patientId, tokenHash: await hash(token), expiresAt: Date.now() + Math.min(expiresIn, 1800) * 1000, nonce: crypto.randomUUID() };
...
export async function verifyReceipt(secret: string, receipt: string, connector: string, patientId: string, token: string): Promise<void> {
  try {
    ...
    if (data.connector !== connector || data.patientId !== patientId || data.expiresAt <= Date.now() || data.tokenHash !== await hash(token)) throw new Error();
...
const referenceSchema = z.object({ connector: z.enum(['atrius','cigna']), patientId: z.string(), tokenHash: z.string(), reference: z.string().max(2000), expiresAt: z.number() }).strict();
export async function signReference(secret: string, connector: 'atrius' | 'cigna', patientId: string, token: string, reference: string): Promise<string> {
  const payload = encode(new TextEncoder().encode(JSON.stringify({ connector, patientId, tokenHash: await hash(token), reference, expiresAt: Date.now() + 5 * 60000 })));
...
export async function verifyReference(secret: string, capability: string, connector: string, patientId: string, token: string, reference: string): Promise<void> {
  try {
    ...
    if (value.reference !== reference || value.connector !== connector || value.patientId !== patientId || value.tokenHash !== await hash(token) || value.expiresAt <= Date.now()) throw new Error();
