...
export default function App() {
  ...
  const connectorNames = useMemo(() => new Map((status?.connectors ?? []).map(connector => [connector.id, connector.name])), [status]);
  const sourceName = (source: string) => connectorNames.get(source) ?? source;
  const patientConnectors = useMemo(() => (status?.connectors ?? []).filter(connector => connector.apiType === 'patient_access'), [status]);
  const matchingConnectors = useMemo(() => patientConnectors.filter(connector => connector.name.toLowerCase().includes(connectorQuery.trim().toLowerCase())), [patientConnectors, connectorQuery]);
      ...
      <main id="main-content" ref={mainRef} tabIndex={-1} className="main-content">
        ...
        {step === 1 && <>
          <section className="card" aria-labelledby="records-heading">
            ...
            {(patientConnectors.length > CONNECTOR_PAGE_SIZE || connectorQuery) && <label className="search-input connector-search"><Search size={17} /><input type="search" aria-label="Search providers and insurers" placeholder="Search providers and insurers" value={connectorQuery} onChange={event => { setConnectorQuery(event.target.value); setConnectorPage(0); }} /></label>}
            {!patientConnectors.length && <p className="connector-note" role="status">{statusBusy ? 'Checking connection availability…' : statusError ? 'Connection status unavailable. Retry availability to check again.' : 'No provider or insurer connections are available yet. You can add your care details below.'}</p>}
            {Boolean(patientConnectors.length) && !matchingConnectors.length && <p className="connector-note" role="status">No providers or insurers match your search.</p>}
            ...
            {(statusError || (status && (!patientConnectors.length || patientConnectors.some(connector => !connector.configured || !connector.enabled)))) && <button className="text-button" disabled={statusBusy} onClick={() => setStatusRetry(current => current + 1)}>{statusBusy ? <><LoaderCircle className="spin" size={15} /> Checking availability…</> : 'Retry availability'}</button>}
