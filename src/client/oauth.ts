...
export async function connectPatient(id: string): Promise<ImportResult> {
  ...
  try {
    ...
    for (const [key, value] of Object.entries({ response_type: 'code', client_id: config.clientId, redirect_uri: config.redirectUri, scope: config.scopes, aud: config.audience, state, code_challenge: codeChallenge, code_challenge_method: 'S256', response_mode: config.responseMode })) authorize.searchParams.set(key, value);
    if (config.scopes.split(/\s+/).includes('openid')) authorize.searchParams.set('nonce', random());
    popup.location.href = authorize.href;
    const code = await codePromise;
    const credentials = await json<{ accessToken: string; patientId: string; receipt: string }>(await fetch(`${connectorPath}/token`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ code, verifier }), signal: controller.signal, cache: 'no-store' }));
    ...
    const resolveReferences = async (references: Reference[]) => {
      ...
      for (let hop = 0; hop < 2 && queue.length; hop++) {
        ...
        for (let i = 0; i < unique.length; i += 10) {
          ...
          const reply = await json<{ resources: unknown[]; references: Reference[]; incomplete: boolean }>(await fetch(`${connectorPath}/references`, {
