...
describe('local connector configuration', () => {
  it('carries Epic aliases and confidential Cigna credentials into private Worker bindings, including explicit empty overrides', async () => {
    ...
    try {
      ...
      expect(connectorConfig(bindings, 'atrius')).toMatchObject({ enabled: true, clientId: env.EPIC_CLIENT_ID, clientSecret: env.EPIC_CLIENT_SECRET });
      ...
      expect(connectorConfig(parseEnv(await readFile(path, 'utf8')), 'atrius').configured).toBe(false);
